...
return new class extends Migration
{
    ...
    {
        $tableName = config('mandate.tables.roles', 'mandate_roles');
        $idType = config('mandate.id_type', 'bigint');
        $addContext = config('mandate.context.roles', false);
        ...
        Schema::create($tableName, function (Blueprint $table) use ($idType, $addContext, $contextMorphName) {
            ...
            if ($addContext) {
                ...
                $table->index(["{$contextMorphName}_type", "{$contextMorphName}_id"], 'roles_context_model_index');
            }
    ...
    {
        Schema::dropIfExists(config('mandate.tables.roles', 'mandate_roles'));
    }
};
